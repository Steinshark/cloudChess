...
def norm(name: str, channels: int) -> nn.Module:
...
class ResidualBlock(nn.Module):
    def __init__(self, channels: int, normalization: str, act: str) -> None:
        super().__init__()
        self.conv1 = nn.Conv2d(channels, channels, 3, padding=1, bias=False)
        self.norm1 = norm(normalization, channels)
        self.act1 = activation(act)
        self.conv2 = nn.Conv2d(channels, channels, 3, padding=1, bias=False)
        self.norm2 = norm(normalization, channels)
...
class ChessPolicyValueNet(nn.Module):
    def __init__(self, config: ModelConfig) -> None:
        ...
        self.stem = nn.Sequential(
            ...
            norm(config.normalization, config.channels),
            activation(config.activation),
        )
        self.tower = nn.Sequential(*[
            ResidualBlock(config.channels, config.normalization, config.activation)
            for _ in range(config.residual_blocks)
        ])
        self.policy_head = nn.Conv2d(config.channels, config.policy_planes, 1)
        self.value_conv = nn.Sequential(
            nn.Conv2d(config.channels, 32, 1, bias=False),
            norm(config.normalization, 32),
            activation(config.activation),
        )
    ...
    def reset_parameters(self) -> None:
        for module in self.modules():
            if isinstance(module, nn.Conv2d):
                ...
                if module.bias is not None:
                    nn.init.zeros_(module.bias)
        nn.init.uniform_(self.value_head[-2].weight, -1e-3, 1e-3)
        nn.init.zeros_(self.value_head[-2].bias)

    def forward(self, x: Tensor) -> tuple[Tensor, Tensor]:
        x = self.tower(self.stem(x))
        policy = self.policy_head(x).permute(0, 2, 3, 1).flatten(1)
